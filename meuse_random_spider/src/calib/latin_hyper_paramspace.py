...
if __name__ == "__main__":
    try:
        # test dev
        work_dir = Path(r'/p/11209265-grade2023/wflow/RWSOS_Calibration/meuse_random')
        
        # find last level from the final level directory
        import glob
        levels = glob.glob(str(Path(r'/p/11209265-grade2023/wflow/RWSOS_Calibration/meuse_random/data/2-interim','calib_data', "level*")))
        print(levels)
        levels_ints = [int(level.split("level")[-1]) for level in levels]
        print(levels_ints)
        last_level = max(levels_ints)
        
        # set up input vars
        # l=setup_logging(work_dir, 'latin_hyper_paramspace.log')
        RECIPE = Path(r'/p/11209265-grade2023/wflow/RWSOS_Calibration/meuse_random/config/LHS_calib_recipe.json')
        N_SAMPLES = 10
        OPTIM = 'random-cd'
