...
start_long, end_long = "1990-01-01", "1999-12-31"
start_1, end_1 = "2000-01-01", "2004-12-31"
start_2, end_2 = "2005-01-01", "2009-12-31"
start_3, end_3 = "2010-01-01", "2014-12-31"

# replace_obs = "ERA5"
include_nwm_data = False
...
for basin in basins:
    ...
    figure_dir = Rf"{main_dir}\docs\images\post_cc_unit\{basin}"

    model_runs = {
        "ERA5": Rf"{main_dir}\models\MODELDATA_{basin}_CLIMATE\run_default\era5_3hourly\output_scalar.nc",
        "EC-Earth": Rf"{main_dir}\models\MODELDATA_{basin}_CLIMATE\run_default\cmip6_EcEarth_historic\output_scalar.nc",
        "GFDL": Rf"{main_dir}\models\MODELDATA_{basin}_CLIMATE\run_default\cmip6_GFDL_historic\output_scalar.nc",
    }
    ...
    for sub in list(map(str, mod_stations)):
        try:
            ds["Q"].loc[dict(runs="Obs.", stations=int(sub))] = df_obs.loc[
                rng, f"Q_{sub}"
            ]
        except Exception:
            tmp = df_obs.loc[:, f"Q_{sub}"]
            start = max(rng[0], tmp.index[0])
            stop = min(rng[-1], tmp.index[-1])
            ds["Q"].loc[
                dict(runs="Obs.", stations=int(sub), time=slice(start, stop))
            ] = tmp.loc[start:stop]
