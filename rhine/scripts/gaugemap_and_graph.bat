...
pixi run python src/1-prepare/update_gauges.py "%cwd%" "%config_root%" "%gauges%" --new_root "%new_root%" --mode "%mode%" --basename "%basename%" --index_col "%index_col%" --snap_to_river True --max_dist %max_dist%  --derive_subcatch True --crs "%crs%" --config_old "%config_old%" --config_new "%config_new%"
