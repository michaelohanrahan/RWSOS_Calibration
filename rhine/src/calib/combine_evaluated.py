...
def main(
    ...
    if level_int == 0:
        ...
        final_df = pd.concat([prev_df, inter_df], axis=0)
        # this way the next phase has access to the best 10 of all levels
    final_df.to_csv(out, index=True)
    ...
    try:
        if 'snakemake' in globals():
            main(
                ...
                random_params=snakemake.input.random_params,
            )
        else:
